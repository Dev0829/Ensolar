...
if (!function_exists('cmplz_activation_check')) {
    ...
    function cmplz_activation_check()
    {
        if (version_compare(PHP_VERSION, '7.2', '<')) {
            ...
            wp_die(__('Complianz cannot be activated. The plugin requires PHP 7.2 or higher', 'complianz-gdpr'));
        }

        global $wp_version;
        if (version_compare($wp_version, '4.9', '<')) {
            deactivate_plugins(plugin_basename(__FILE__));
            wp_die(__('Complianz cannot be activated. The plugin requires WordPress 4.9 or higher', 'complianz-gdpr'));
        }
    }
