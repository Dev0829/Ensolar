    ...
    class cmplz_license
    {
        ...
        public function add_license_warning( $warnings ){
        	...
        	if ( empty($status) ){
				...
				if ( cmplz_is_multisite_plugin_on_non_multisite_installation() ) {
					$text = cmplz_sprintf( __( 'You have activated the Multisite plugin on a non-Multisite environment. Please download the regular Complianz Premium plugin %svia your account%s and install it instead', 'complianz-gdpr' ), '<a href="https://complianz.io/account">', '</a>' );
				} else {
					$text = cmplz_sprintf( __( 'Please check your %slicense status%s.', 'complianz-gdpr' ), $license_link, "</a>" );
				}
		...
		public function activate_license()
		{
			...
			if ( $auto_installed_license || isset($_POST['cmplz_license_activate']) ) {
				...
				$license = $auto_installed_license ?: trim(sanitize_text_field($_POST['cmplz_license_key']));
		...
		public function add_multisite_menu()
		{
			...
			add_submenu_page( 'settings.php', "Complianz", "Complianz",
        ...
        public function ms_dismiss_license_notice_get(){
			if (isset($_GET['cmplz_dismiss_license_notice'])){
		        update_option('cmplz_license_notice_dismissed', true, false );
	        }
        }
		...
		public function get_license_label(){
			...
			if ( $activation_limit == 0 && $status === 'valid' ) {
