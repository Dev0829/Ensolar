...
function cmplz_tcf_init() {
	if ( cmplz_iab_is_enabled() ) {
		...
		add_action( 'cmplz_upgrade',  'cmplz_tcf_upgrade_premium', 10);
...
function cmplz_tcf_upgrade_premium($prev_version) {
...
function cmplz_tcf_override_auto_updates( $update, $item ) {
	...
	if ( isset( $item->slug ) && strpos( $item->slug , 'complianz-gpdr-premium' ) !==false ) {
		if ( is_multisite() ) {
			$option = get_site_option('auto_update_plugins',[]);
		} else {
			$option = get_option('auto_update_plugins',[]);
		}
		if ( !in_array('complianz-gdpr-premium/complianz-gpdr-premium.php', $option) ){
...
function cmplz_tcf_cmp_files_missing(){
	...
	$path       = $upload_dir . 'cmp/vendorlist/';
	return ! file_exists( $path . 'vendor-list.json' ) || get_option( 'cmplz_fallback_cmp' );
}
/**
...
function cmplz_update_json_files() {
	if ( cmplz_iab_is_enabled() ) {
		$cmplzExistingLanguages = ['gl','eus','bg', 'ca', 'cs', 'da', 'de', 'el', 'es', 'et', 'fi', 'fr', 'hr', 'hu', 'it', 'ja', 'lt', 'lv', 'mt', 'nl', 'no', 'pl', 'pt', 'ro', 'ru', 'sk', 'sl', 'sr', 'sv', 'tr', 'zh',];
...
function cmplz_download_json_to_site( $src ) {
	...
	if ( ! file_exists( $upload_dir . 'cmp' ) ) {
		mkdir( $upload_dir . 'cmp' ,0755);
	}

	if ( ! file_exists( $upload_dir . 'cmp/vendorlist' ) ) {
		mkdir( $upload_dir . 'cmp/vendorlist' ,0755);
	}

	//download file
	$tmpfile  = download_url( $src, $timeout = 25 );
	$file     = $upload_dir . "cmp/vendorlist/" . basename( $src );
...
function cmplz_tcf_change_settings() {
	...
	if ( !get_option("cmplz_tcf_initialized") ) {
		...
		cmplz_update_option( 'settings', 'a_b_testing_buttons', false );
		/**
		 * Send an email
		 * but only once
		 */
...
function cmplz_tcf_settings_html($data, $banner){
	...
	$data['message_optout'] = cmplz_get_default_banner_text();
	return $data;
}
...
function cmplz_tcf_edit_cookiebanner_settings($fields){
	$fields['a_b_testing_buttons']['disabled'] = true;
...
function cmplz_tcf_us_vendors($atts = array(), $content = null, $tag = ''){
	...
	$html =
		'<div id="cmplz-tcf-wrapper-nojavascript">'.__("The TCF vendorlist is not available when javascript is disabled or are dismissed by browser privacy preferences.","complianz-gdpr").'</div>' .
		'<div id="cmplz-tcf-wrapper">
			<div id="cmplz-tcf-vendor-template" class="cmplz-tcf-template">'.$template.'</div>
...
function cmplz_tcf_vendors( $atts = array(), $content = null, $tag = '' ) {
	...
	$html =
		'<div id="cmplz-tcf-wrapper-nojavascript">'.__("The TCF vendorlist is not available when javascript is disabled or are dismissed by browser privacy preferences.","complianz-gdpr").'</div>' .
		'<div id="cmplz-tcf-wrapper">'.$buttons.'
			<div id="cmplz-tcf-vendor-template" class="cmplz-tcf-template">'.$template.'</div>
			<div id="cmplz-tcf-type-template" class="cmplz-tcf-template">'.$type_template.'</div>
		<p>'.
		...
		<p class="cmplz-subtitle">7.2.2 '.__("Special features and purposes","complianz-gdpr").'</p>
		...
		<p class="cmplz-subtitle">7.2.3 '.__("Vendors","complianz-gdpr").'</p>
