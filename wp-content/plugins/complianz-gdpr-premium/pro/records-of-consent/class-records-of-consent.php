	...
	class cmplz_records_of_consent {
		...
		public function ajax_export_roc_to_csv(){
			...
			$response   = json_encode( array(
				...
				'link' => '<a class="cmplz-button cmplz-button-primary cmplz-header-btn" href="'.$this->fileurl()."?token=".time().'">'.__("Download", "complianz-gdpr").'</a>',
			) );
			header( "Content-Type: application/json" );
			echo $response;
			exit;
		...
		private function filepath(){
			$upload_dir = cmplz_upload_dir();
			return $upload_dir .get_option('cmplz_roc_file_name').".csv";
		}
