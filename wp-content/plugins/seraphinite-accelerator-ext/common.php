...
function OnAsyncTasksGetPushUrlFile()
{
	...
	return( Gen::GetArrField( $settGlob, array( 'asyncUseCron' ), true ) ? 'wp-cron.php' : 'index.php' );
}

function OnAsyncTasksPushByRe()
{
	$settGlob = Plugin::SettGetGlobal();
	if( !Gen::GetArrField( $settGlob, array( 'asyncUseRe' ), false ) )
		return( false );

	$sPhpLauncherProbe = Gen::GetArrField( $settGlob, array( 'reLnch' ), '' );
	return( $sPhpLauncherProbe ? $sPhpLauncherProbe : true );
}
		...
		'contPr' => array(
			...
			'css' => array(
				...
				'custom' => array(
					...
					'owl-carousel'	=> array( 'enable' => true,		'descr' => 'OWL Carousel',				'data' => ".owl-carousel:not(.owl-loaded) {\n\tdisplay:block!important;\n}\n\n.owl-carousel:not(.owl-loaded) > *:not(:first-child) {\n\tdisplay:none;\n}\n\n.owl-carousel .container.full-screen {\n\theight: 100vh;\n}" ),
...
body.royal_preloader {
	visibility: hidden !important;
					...
					'elementor-vis'		=> array( 'enable' => true,		'descr' => 'Elementor (visibility and animation)',					'data' => "/*body.seraph-accel-js-lzl-ing-ani .elementor-invisible {\n\tvisibility: visible!important;\n}*/\n\n/*.elementor-element[data-settings*=\"animation\\\"\"] {\n\tanimation-name: none!important;\n}*/" ),
...
body.seraph-accel-js-lzl-ing-ani .elementor-headline-dynamic-text.elementor-headline-text-active {
	opacity: 1;
					...
					'et'			=> array( 'enable' => true,		'descr' => 'Divi',						'data' => '.et_animated {
	opacity: 1 !important;
}
...
.et_pb_slider:not([data-active-slide]) .et_pb_slide:first-child .et_pb_container {
	height: 100%;
					...
					'rev-slider'		=> array( 'enable' => true,		'descr' => 'Revolution Slider',		'data' => "rs-module[data-lzl-layout=\"fullwidth\"] {\n\twidth: var(--seraph-accel-client-width);\n\tleft: calc((100% - var(--seraph-accel-client-width)) / 2);\n}\n\nrs-module[data-lzl-layout=\"fullscreen\"] {\n\twidth: var(--seraph-accel-client-width);\n\tleft: calc((100% - var(--seraph-accel-client-width)) / 2);\n\theight: 100vh !important;\n}" ),
					'fusion-vis'		=> array( 'enable' => true,		'descr' => 'Fusion (visibility and animation)',					'data' => ".fusion-animated {\n\tvisibility: visible;\n}\n" ),
...
function GetCacheCurUserSession( $siteId )
{
...
function CacheCgf( $dataPath, $oiCi, $fileExt, $dataFileExt = '' )
{
	return( $dataPath . '/' . $oiCi . '.' . $fileExt . $dataFileExt );
}
...
function CacheCw( $settCache, $siteRootPath, $dataPath, $composite, $content, $type, $fileExt = null )
{
	...
	foreach( $dataComprs as $dataCompr )
	{
		...
		$oiCf = $dataPath . '/' . $oiCi . '.' . $fileExt . $dataFileExt;
...
function Tof_GetFileData( $dir, $id, $ver = null, $compressed = false, $oiCi = null )
{
	...
	if( !is_array( $data ) || ( $ver && $ver !== (isset($data[ 'v' ])?$data[ 'v' ]:null) ) )
		$data = array();
...
function CheckPathInUriList( $a, $path )
{
	foreach( $a as $e )
	{
		_NormalizeExclPath( $e );

		if( IsStrRegExp( $e ) ? preg_match( $e, $path ) : ( strpos( $path, $e ) === 0 ) )
			return( $e );
	}
...
function ContProcGetSkipStatus( $content )
{
	...
	if( PluginLic::CheckFeature() != Gen::S_OK )
		return( $seraph_accel_g_contProcGetSkipStatus = 'outOfLicense' );
...
{
	return( 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.20.31 ' . ucwords( implode( ' ', Gen::GetArrField( $viewsDeviceGrp, array( 'agents' ), array() ) ) ) );
}
...
function OnAsyncTask_CacheProcessItem( $args )
{
...

	{
		...
		if( OnAsyncTasksPushByRe() )
			$requestRes = PluginRe::MakeRequest( 'GET', $url, $hdrsForRequest, 30, false );
		else

			$requestRes = Wp::RemoteGet( $url, array( 'redirection' => 0, 'timeout' => 30, 'sslverify' => false, 'headers' => $hdrsForRequest ) );
...
function GetExtContents( $url, &$contMimeType = null, $userAgentCmn = true, $timeout = 30, $rememberServerState = true )
{
	...
	$args = array( 'sslverify' => true, 'timeout' => $timeout );
	if( $userAgentCmn )
		$args[ 'user-agent' ] = 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.20.31';
...
{
	Gen::LogWrite( GetCacheDir() . '/logs/log.txt', $text, $severity, $category );
}
