...
function OnAdminApi_SelfDiag_CronExec( $args )
{
	$res = new AnyObj();
	$res -> hr = Gen::S_OK;
	...
	$url = Wp::GetCronUrl( array( '_' => Gen::MicroTimeStamp() ) );
	...
	else if( stripos( wp_remote_retrieve_body( $requestRes ), 'error' ) !== false )
	{
		$res -> hr = Gen::E_FAIL;
		$res -> descr = _SelfDiag_GetResponseResString( $url, $requestRes, true );
		return( $res );
	}

	PluginFileValues::Set( 'selfDiag_test', true );

	if( wp_next_scheduled( 'seraph_accel_selfDiag_cronTestTask' ) === false )
	{
		$setHookRes = wp_schedule_single_event( time() - 10, 'seraph_accel_selfDiag_cronTestTask', array(), true );
		if( is_wp_error( $setHookRes ) )
		{
			$res -> hr = Gen::E_FAIL;
			$res -> descr = $setHookRes -> get_error_message();
		}
	}
	...
	if( $res -> hr === Gen::S_OK )
	{
		while( $timeout )
		{
			...
			$url = add_query_arg( array( '_' => Gen::MicroTimeStamp() ), Wp::GetSiteRootUrl() );
			$requestRes = Wp::RemoteGet( $url, array( 'timeout' => 45, 'sslverify' => false ) );
			$res -> hr = Net::GetHrFromWpRemoteGet( $requestRes );
			if( $res -> hr !== Gen::S_OK )
			{
		...
		if( $res -> hr == Gen::S_OK && !$timeout )
		{
			$res -> hr = Gen::S_FALSE;
			$res -> descr = Wp::safe_html_x( 'CronExecTimeoutExpired', 'admin.SelfDiag', 'seraphinite-accelerator' );
		}
	}
...
function OnAdminApi_SelfDiag_PageOptimize( $args )
{
	...
	$requestRes = OnAsyncTasksPushByRe() ? PluginRe::MakeRequest( 'GET', $url, array(), 45 ) : Wp::RemoteGet( $url, array( 'timeout' => 45, 'sslverify' => false ) );
...
function SelfDiag_DetectStateAnd3rdPartySettConflicts( $cb, $ext = false )
{
	...
	if( $isContProcEnabled )
	{
		...
		if( !Gen::DoesFuncExist( 'imagecreatefromstring' ) )
		{
			...
			if( Gen::GetArrField( $sett, 'contPr/img/szAdaptBg', false, '/' ) )
				call_user_func_array( $cb, array( Ui::MsgWarn, sprintf( Wp::safe_html_x( 'PhpGdNotActive_%1$s%2$s%3$s', 'admin.Notice', 'seraphinite-accelerator' ), 'GD',
					sprintf( Plugin::GetPluginString( 'SubjectTitle_%1$s%2$s' ), esc_html_x( 'Lbl', 'admin.Settings_Images_Adapt', 'seraphinite-accelerator' ), sprintf( esc_html_x( 'BgSzChk_%1$s', 'admin.Settings_Images_Adapt', 'seraphinite-accelerator' ), Gen::GetArrField( $sett, 'contPr/img/szAdaptBgCxMin', 0, '/' ) ) ),
					Ui::Link( esc_html_x( 'Title', 'admin.Settings_Images', 'seraphinite-accelerator' ), menu_page_url( 'seraph_accel_settings', false ) . '#images' ) ) ) );
		}
	}
	...
	if( $theme )
	{
		...
		case 'dt-the7':
			...
			if( $isScriptsDelayLoadEnabled && _3rdParty_to_boolean( Gen::GetArrField( $themeOpts, 'general-beautiful_loading', null, '/' ) ) )
				call_user_func_array( $cb, array( Gen::SevWarn, sprintf( Wp::safe_html_x( '3rdSett_DisplayConflict_%1$s%2$s%3$s%4$s', 'admin.Notice', 'seraphinite-accelerator' ), $contRemindRefreshCache, $theme -> name, Wp::GetLocString( 'Beautiful loading', 'theme-options', 'the7mk2' ), esc_html_x( 'OffOptionValue', 'admin.Notice', 'seraphinite-accelerator' ) ) ) );
