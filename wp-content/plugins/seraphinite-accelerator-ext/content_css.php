	...
	if( $aImgSzAlternativesBlocksGlobal )
	{
		$cssCritDoc = new Sabberworm\CSS\CSSList\Document();
			...
			foreach( $aImgSzAlternativesBlocksGlobal as $aImgSzAlternativesBlocks )
				$cssCritDoc -> append( StyleProcessor::ImgSzAlternativesGetStyleBlocks( $aImgSzAlternativesBlocks[ 'sels' ], $aImgSzAlternativesBlocks[ 'alts' ], true, $aImgSzAlternativesBlocks[ 'isLazyBg' ] ) );
...
class StyleProcessor
{
	...
	static function ImgLazyBgGetStyleBlocks( $aSel, $lazyBg, $important = false )
	{
		$ruleAdd = new Sabberworm\CSS\Rule\Rule( 'background-image' );
		$ruleAdd -> setValue( $lazyBg -> info ? new Sabberworm\CSS\Value\URL( new Sabberworm\CSS\Value\CSSString( LazyLoad_SrcSubst( $lazyBg -> info, true ) ) ) : 'none' );
	...
	static function ImgSzAlternativesGetStyleBlocks( $aSel, $imgSzAlternatives, $important = false, $bLazyBg = false )
	{
		$aBlock = array();
		...
		if( $bLazyBg )
			Gen::ArrAdd( $aBlock, StyleProcessor::ImgLazyBgGetStyleBlocks( $aSel, new AnyObj( array( 'info' => $imgSzAlternatives -> info, 'isImportant' => $important ) ), $important ) );

		return( $aBlock );
	}

	private function _AdjustContBlock( $stage, &$aCritFonts, $block, &$blockParents, &$blockParentsCrit, &$blockParentsFonts, &$isCritDocAdjusted, &$isFontsDocAdjusted, $extract, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, $isInlined )
	{
		$isAdjusted = null;
		$canMoveTo = false;
		...
		else if( $block instanceof Sabberworm\CSS\RuleSet\DeclarationBlock )
		{
			if( !$stage || $stage == 'main' )
			{
				...
				if( !$block -> isEmpty() )
				{
					...
					if( $this -> _AdjustBlock( $block, $extract, $isCrit, $aCritFonts, $ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, $isInlined, $adjustes ) )
						$isAdjusted = true;

					if( $adjustes -> imgSzAlternatives && $adjustes -> imgSzAlternatives -> a )
						Gen::ArrAdd( $aBlockAddAfter, StyleProcessor::ImgSzAlternativesGetStyleBlocks( $block -> getSelectors(), $adjustes -> imgSzAlternatives, false, !!$adjustes -> lazyBg ) );
					else if( $adjustes -> lazyBg )
						Gen::ArrAdd( $aBlockAddAfter, StyleProcessor::ImgLazyBgGetStyleBlocks( $block -> getSelectors(), $adjustes -> lazyBg ) );
		...
		if( $isCrit )
		{
			...
			if( $aBlockAddAfter )
				$blockParentsMoveTo[ count( $blockParentsMoveTo ) - 1 ] -> append( $aBlockAddAfter );
		}
		else
		{
			if( $aBlockAddAfter )
				$blockParents[ count( $blockParents ) - 1 ] -> insert( $aBlockAddAfter, $block );
		}
	...
	private function _AdjustBlock( $ruleSet, $extract, &$isCrit, &$aCritFonts, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, $isInlined, $adjustes )
	{
		if( !$extract )
			$isCrit = true;
		...
		if( (isset($ctxItems -> lazyBg)?$ctxItems -> lazyBg:null) || ( $adjustes -> imgSzAlternatives && $adjustes -> imgSzAlternatives -> a ) )
			foreach( $selectors as $sel )
		...
		if( !$isCrit )
			foreach( $selectors as $sel )
			{
				if( $this -> isCssSelCrit( $ctxProcess, $sel ) )
				{
					$isCrit = true;
					break;
				}
