...
function _Process( $sites, $args )
{
	...
	$viewId = GetCacheViewId( $seraph_accel_g_ctxCache, $settCache, $userAgent, $args );
	...
	if( !$dscFileTm || ( $timeoutCln > 0 && $dscFileTmAge > $timeoutCln && ( $dscFileTm >= 60 ) ) || ( $timeout > 0 ? ( $dscFileTmAge > $timeout ) : ( $dscFileTm < 60 ) ) || ( $tmCur > $sessExpiration ) || ( $seraph_accel_g_ctxCache -> isUserSess && $httpCacheControl == 'no-cache' && Gen::GetArrField( $settCache, array( 'ctxCliRefresh' ), false ) ) )
	{
		...
		if( $dscFileTm === false )
		{
			...
			if( $ccs )
			{
				...
				if( $seraph_accel_g_prepPrms !== null )
					$seraph_accel_g_prepPrms[ 'lazyInvTmp' ] = true;
...
function _ProcessOutHdrTrace( $sett, $bHdr, $bLog, $state, $data = null, $dscFile = null )
{
	...
	if( $bHdr )
		@header( 'X-Seraph-Accel-Cache: 2.20.31;' . $debugInfo );

	if( $bLog )
		LogWrite( $debugInfo . ' URL: ' . GetCurRequestUrl() . '; Agent: ' . $userAgent . '; IP: ' . (isset($_SERVER[ 'REMOTE_ADDR' ])?$_SERVER[ 'REMOTE_ADDR' ]:'<UNK>'), Ui::MsgInfo, 'HTTP trace' );
}
...
function _ProcessOutCachedData( $allowExtCache, $objSubType, $sett, $settCache, $dsc, $dscFileTm, $tmCur, $stateValidate, $reason, $out, &$output = null )
{
	...
	if( (isset($settCache[ 'chkNotMdfSince' ])?$settCache[ 'chkNotMdfSince' ]:null) )
	{
		...
		else if( isset( $_SERVER[ 'HTTP_IF_MODIFIED_SINCE' ] ) )
		{
			...
			$bNotMdf = ( strtotime( preg_replace( '@;.*$@', '', $_SERVER[ 'HTTP_IF_MODIFIED_SINCE' ] ) ) == $tmLm );
		}
	}
...
function GetCacheViewId( $ctxCache, $settCache, $userAgent, &$args )
{
	...
	if( (isset($settCache[ 'normAgent' ])?$settCache[ 'normAgent' ]:null) )
	{
		...
		$_SERVER[ 'HTTP_USER_AGENT' ] = 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.20.31';
	}
		...
		if( Gen::GetArrField( $settCache, array( 'viewsGeo', 'enable' ) ) )
		{
			...
			if( !is_string( $ctxCache -> viewGeoId ) )
			{
				...
				foreach( Gen::GetArrField( $settCache, array( 'viewsGeo', 'grps' ), array() ) as $grpId => $grp )
				{
					...
					if( $matched )
					{
						$ctxCache -> viewGeoId = $grpIsFirst ? '' : $grpId;
				...
				if( $ctxCache -> viewGeoId === null )
					$ctxCache -> viewGeoId = $countryCode;
...
function _OutCompressedImage( $sett, $file, $nonce )
{
	...
	if( (isset($sett[ 'hdrTrace' ])?$sett[ 'hdrTrace' ]:null) )
		@header( 'X-Seraph-Accel-Cache: 2.20.31; state=' . ( $mimeType == Fs::MimeTypeDef ? 'original' : 'preoptimized' ) . '; redir=alt;' . ( $aSkip ? ( '; skip=' . @json_encode( $aSkip ) ) : '' ) . ( $mimeType != Fs::MimeTypeDef ? ( '; sizeOrig=' . @filesize( $fileOrig ) ) : '' ) );
