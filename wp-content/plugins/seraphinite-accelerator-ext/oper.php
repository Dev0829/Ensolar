	...
	if( $cbData )
	{
		...
		if( Gen::DirEnum( $cacheDataDir, $ctx,
			function( $path, $item, &$ctx )
			{
				...
				$itemId = '' . (isset($itemId[ 0 ])?$itemId[ 0 ]:null) . '.' . (isset($itemId[ 1 ])?$itemId[ 1 ]:null);

				$itemType = substr( $path, $ctx[ 'cacheDataDirLen' ] + 1 );
				return( call_user_func_array( $ctx[ 'cbData' ], array( &$ctx[ 'ctxWalk' ], $itemType ? $itemType : 'html', $itemId, $item ) ) );
			}
		, true ) === false )
		{
			return( false );
...
function CacheOp( $op, $priority = 0 )
{
	$ctx = new AnyObj();
	$ctx -> op = $op;
	$ctx -> pluginFileValueName = ( $ctx -> op === 1 ) ? 'cln' : 'o';

	if( PluginFileValues::Get( $ctx -> pluginFileValueName ) !== null )
		return( false );
	...
	if( $op == 1 || $op == 2 )
	{
		foreach( glob( GetCacheViewsDir( GetCacheDir() . '/s/' . $curSiteId ) . '*' ) as $viewDir )
		{
...
function CacheOpGetViewsHeaders( $settCache, $viewId = null )
{
	...
	if( $viewId === null || $viewId === 'cmn' )
		$res[ 'cmn' ] = array( 'User-Agent' => 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.20.31' );
...
function CacheVerifyEnvDropin( $sett, $verifyEnvDropin = null )
{
	...
	$verifyEnvDropin -> needed = GetAdvCacheFileContent( $sett );
	$verifyEnvDropin -> actual = @file_get_contents( WP_CONTENT_DIR . '/advanced-cache.php' );
