...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.20.31; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
	...
	if( $ctxProcess[ 'mode' ] == 'full' )
	{
		...
		if( (isset($ctxProcess[ 'lazyload' ])?$ctxProcess[ 'lazyload' ]:null) || (isset($ctxProcess[ 'imgAdaptive' ])?$ctxProcess[ 'imgAdaptive' ]:null) )
		{
...

			{
				...
				if( (isset($ctxProcess[ 'imgAdaptive' ])?$ctxProcess[ 'imgAdaptive' ]:null) )
				{
					...
					$cont .= "(function(g,h){function q(e){var a=[e.clientWidth,e.clientHeight];if(a[0]&&a[1]){var f=getComputedStyle(e),b=f.getPropertyValue(\"--ai-bg-sz\").split(\" \");if(b[1]){var c=f.getPropertyValue(\"background-size\"),d={};\"auto\"==c?d.cx=b[0]:\"contain\"==c?(c=a[0]/a[1],b=b[0]/b[1],d.cx=b>c?a[0]:Math.round(b*a[1])):\"cover\"==c?(c=a[0]/a[1],b=b[0]/b[1],d.cx=c>b?a[0]:Math.round(b*a[1])):(c=c.split(\" \")[0],c.lastIndexOf(\"%\")==c.length-1?d.cx=Math.round(parseInt(c,10)/100*a[0]):d.cx=999999999);\"50%\"==f.getPropertyValue(\"background-position-x\")&&\n(d.cxCropped=a[0]);a=d}else a=void 0}else a=void 0;f=\"-\";if(a){const l=[1920,1366,992,768,480,360];d=void 0;for(b=0;b<l.length;b++){c=l[b];if(a.cxCropped)for(var m=0;m<l.length;m++){var n=l[m];if(a.cxCropped>n)break;if(!d||n<d)f+=(d?d:\"O\")+\"@\"+n+\"-\"}if(a.cx>c)break;f+=c+\"-\";d=c}}else f+=\"0-\";e.getAttribute(\"data-ai-bg\")!=f&&e.setAttribute(\"data-ai-bg\",f)}function k(){p||(p=setTimeout(function(){p=void 0;r()},250))}function r(){for(var e=0;e<t.length;e++)q(t[e])}h.lzl_lazysizesConfig.beforeCheckElem=\nfunction(e){e.classList.contains(\"ai-bg\")&&q(e)};var t=g.getElementsByClassName(\"ai-bg\"),p;g.addEventListener(\"DOMContentLoaded\",r,!1);h.addEventListener(\"hashchange\",k,!0);h.addEventListener(\"resize\",k,!1);h.MutationObserver?(new h.MutationObserver(k)).observe(g.documentElement,{childList:!0,subtree:!0,attributes:!0}):(g.documentElement.addEventListener(\"DOMNodeInserted\",k,!0),g.documentElement.addEventListener(\"DOMAttrModified\",k,!0))})(document,window);";
				}

				if( $bLazyCont )
				{

					$cont .= str_replace( array( '_URL_GET_CONTPARTS_' ), array( ContentProcess_GetGetPartUri( $ctxProcess, '{id}.html' ) ), ";(function(e,g){function n(a,b,d,c){var f=b.getAttribute(\"data-lzl-nos\");if(f)if(a=b.getAttribute(\"data-cp\"))c&&h++,d=new g.XMLHttpRequest,d.open(\"GET\",\"_URL_GET_CONTPARTS_\".replace(\"%7Bid%7D\",a),!0),d.onload=function(){if(200==this.status)try{b.outerHTML=this.responseText}catch(k){}c&&(h--,!h&&l&&(l(),l=void 0))},d.send();else if(a=b.parentNode.querySelector('noscript[data-lzl-nos=\"'+f+'\"]'))b.outerHTML=a.textContent,a.parentNode.removeChild(a),d&&d.fire(g,\"resize\",{},!1,!0)}function m(a,b,d){if(void 0!==\nb){if(\"string\"!==typeof b)return;var c=b.indexOf(\"#\");if(-1==c)return;c=b.substr(c+1)}if(void 0===c||!a.querySelector('[id=\"'+c+'\"]')){b=g.lzl_lazySizes;for(var f=a.querySelectorAll(\"i\"+(d?\".bjs\":\".lzl\")+\"[data-lzl-nos]\"),k=0;k<f.length;k++){var p=f[k];p.classList.remove(\"lzl\");n(a,p,b,d);if(void 0!==c&&a.querySelector('[id=\"'+c+'\"]'))break}}}var h=0,l;g.lzl_lazysizesConfig.beforeUnveil=function(a,b){n(a.ownerDocument,a,b,a.classList.contains(\"bjs\"))};m(e,location.href);e.addEventListener(\"click\",\nfunction(a){m(e,a.target.getAttribute(\"href\"))},{capture:!0,passive:!0});e.addEventListener(\"keydown\",function(a){(70==a.keyCode&&(a.ctrlKey||a.metaKey)||191==a.keyCode)&&m(e)},{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(a){m(e,void 0,!0);if(h)return l=a,!0},5)})(document,window);" );
				}
		...
		if( $aFreshItemClassApply )
		{
			...
			else
			{
				...
				HtmlNd::SetValFromContent( $item, str_replace( array( '_URL_GET_FRESH_', '_ARRAY_SELECTORS_' ), array( ContentProcess_GetCurRelatedUri( $ctxProcess, array( 'seraph_accel_gf' => '{tm}' ) ), implode( ',', array_map( function( $v ) { return( '"' . $v . '"' ); }, $aFreshItemClassApply ) ) ), "(function(a,m,h){var c=new m.XMLHttpRequest,k=function(){};seraph_accel_izrbpb.add(function(d){if(c)return k=d,!0},5);c.open(\"GET\",\"_URL_GET_FRESH_\".replace(\"%7Btm%7D\",\"\"+Date.now()),!0);c.onload=function(){function d(g=!0){g&&a.removeEventListener(h,d);[_ARRAY_SELECTORS_].forEach(function(e){var f='[data-lzl-fr=\"'+e+'\"]';e=a.querySelectorAll(f);f=l.querySelectorAll(f);for(var b=0;b<e.length;b++)b<f.length&&(e[b].innerHTML=f[b].innerHTML),e[b].classList.remove(\"lzl-fr-ing\")});c=void 0;g=a.createEvent(\"Events\");\ng.initEvent(\"seraph_accel_freshPartsDone\",!0,!1);a.dispatchEvent(g);k()}var l=a.implementation.createHTMLDocument(\"\");200==this.status&&(l.documentElement.innerHTML=this.responseText);\"loading\"!=a.readyState?d(!1):a.addEventListener(h,d,!1)};c.send()})(document,window,\"DOMContentLoaded\")" ) );
				$ctxProcess[ 'ndHead' ] -> insertBefore( $item, $ctxProcess[ 'ndHead' ] -> firstChild );
				ContentMarkSeparate( $item );
			}
