...
{
	add_menu_page( Plugin::GetPluginString( 'TitleLong' ), Plugin::GetNavMenuTitle(), 'manage_options', 'seraph_accel_manage',																		$accepted ? 'seraph_accel\\_ManagePage' : 'seraph_accel\\Plugin::OutputNotAcceptedPageContent', Plugin::FileUri( 'icon.png?v=2.20.31', __FILE__ ) );
	...
	add_action( 'admin_init',
		function()
		{
			...
			if( isset( $_REQUEST[ 'seraph_accel_saveWizSettings' ] ) )
			{
				...
				Plugin::ReloadWithPostOpRes( array( 'saveSettings' => _OnFinishSettingsWizard( $_POST ) ), menu_page_url( 'seraph_accel_settings', false ) );
				exit;
			}
...
function OnInit( $isAdminMode )
{
	...
	if( $cacheEnable )
	{
		...
		if( Gen::GetArrField( $sett, array( 'cache', 'ctx' ), false ) )
		{
	...
	if( $cacheEnable && Gen::GetArrField( $sett, array( 'cache', 'ctx' ), false )  )
	{
		add_action( 'init',
			function()
			{
...

	{
		...
		Ui::PostBoxes_MetaboxAdd( 'queue', esc_html_x( 'Title', 'admin.Manage_Queue', 'seraphinite-accelerator' ) . Ui::Tag( 'span', Ui::AdminHelpBtn( Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Help.Manage_Queue' ), Ui::AdminHelpBtnModeBlockHeader ) ), true,
			function( $callbacks_args, $box )
			{
				...
				{
					echo( Ui::Tag( 'p',
						Ui::Tag( 'div', GetQueueContent( 0 ) . Ui::Tag( 'div', Ui::Spinner( true, array( 'style' => array( 'vertical-align' => 'middle', 'margin-top' => '2em' ) ) ), array( 'style' => array( 'text-align' => 'center' ) ) ), array( 'class' => 'content seraph_accel_textarea', 'style' => array( 'overflow' => 'scroll', 'min-height' => '7em', 'height' => '15em', 'max-height' => '30em', 'resize' => 'vertical' ) ) ),
						array(  )
					) );
...
function CacheInitOperScheduler( $force = false, $init = true )
{
	...
	{
		if( !(isset($item[ 'enable' ])?$item[ 'enable' ]:null) || !(isset($item[ 'deps' ])?$item[ 'deps' ]:null) )
			continue;
...
function OnAsyncTask_CacheNextScheduledOp( $args )
{
	...
	foreach( $aId as $id => $op )
		foreach( Gen::GetArrField( $settSche, array( $id, 'deps' ), array() ) as $url )
			$aUrls[ $op ][] = $url;

	foreach( $aUrls as $op => $urls )
	{
		if( Gen::GetArrField( Plugin::SettGet(), array( 'log' ), false ) && Gen::GetArrField( Plugin::SettGet(), array( 'logScope', 'upd' ), false ) )
		{
			$txt = '';
		...
		if( CacheOpUrls( true, $urls, $op, 7, $cbIsAborted ) === false )
			break;
	}
...
function GetQueueContent( $nMaxItems, &$nums = null, $siteId = null )
{
	...
	$isUserCacheEnabled = Gen::GetArrField( $sett, array( 'cache', 'ctx' ), false );
	...
	if( $nMaxItems )
	{
		...
		foreach( $items as $itemState => $itemsPerState )
		{
			foreach( $itemsPerState as $item )
			{
				...
				case 7:			$initiator = Wp::safe_html_x( 'InitiatorSpecSche', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
...
{
	Gen::LogClear( GetCacheDir() . '/logs/log.txt' );
}
