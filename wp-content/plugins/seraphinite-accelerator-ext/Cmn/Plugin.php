...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.20.31' || $lastCheckPackage !== 'Full' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.20.31' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.20.31' )
				return( Gen::S_FALSE );
...
class Plugin
{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.20.31' ), '2.20.31' ) > 0 )
					{
	...
	static function ReloadWithPostOpRes( $res, $redir = null )
	{
		...
		wp_safe_redirect( add_query_arg( array( 'seraph_accel_postOpsRes' => rawurlencode( base64_encode( json_encode( $res ) ) ) ), $redir ) );
	}
	...
	static function AsyncTaskPushEx( $urlPush, $timeout = Plugin::ASYNCTASK_PUSH_TIMEOUT )
	{
		...
		if( Gen::CallFunc( 'seraph_accel\\OnAsyncTasksPushByRe' ) )
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.20.31' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.20.31' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.20.31' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static function Sett_SaveBtn( $name, $full = true )
	{
		...
		$res .=
			...
			Ui::InputBox( 'file', 'seraph_accel_settings_import_file', null, array( 'style' => array( 'display' => 'none' ), 'accept' => '.json', 'onchange' => 'seraph_accel.PluginAdmin.SettImport(this,"' . $urlAct . '",this.files.length?this.files[0]:null)' ) ) .
			'';
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.20.31' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.20.31.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.20.31' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.20.31' ) > 0 )
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.20.31' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.20.31' );
			if( $id == 'Gen' )
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.20.31" ) );
...
class PluginRe
{
	...
	static function Exec( $sMethod, $sUrl, $aHdrs, $nWaitForEnd = true, $bBody = true, &$sBody = null )
	{
		...
		$sCmd = Gen::ExecEscArg( $sPhpLauncher ) . ' -c ' . Gen::ExecEscArg( php_ini_loaded_file() ) . ' -d opcache.file_cache=' . Gen::ExecEscArg( $dirOpCache ) . ' -d opcache.enable_cli=1 -d opcache.file_cache_only=1 -d opcache.optimization_level=' . ini_get( 'opcache.optimization_level' ) . ' -d display_errors=' . ini_get( 'display_errors' ) . ' -d output_buffering=' . ini_get( 'output_buffering' ) . ' -d register_argc_argv=1 ' . Gen::ExecEscArg( dirname( __DIR__ ) . '/re.php' ) . ( is_int( $nWaitForEnd ) ? ( ' --t=' . $nWaitForEnd ) : '' ) . ( $fileOutTmt ? ( ' --tmp=' . Gen::ExecEscArg( $fileOutTmt ) ) : '' ) . ' --root=' . Gen::ExecEscArg( $_SERVER[ 'DOCUMENT_ROOT' ] ) . ' --soft=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( (isset($_SERVER[ 'SERVER_SOFTWARE' ])?$_SERVER[ 'SERVER_SOFTWARE' ]:'') ) ) ) . ' --method=' . Gen::ExecEscArg( $sMethod ) . ' --url=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( $sUrl ) ) . ' --hdrs=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( @json_encode( ( array )$aHdrs ) ) ) );
	...
	static private function _GetPhpLauncher()
	{
		...
		if( !$sPhpLauncher )
		{
			$sPhpLauncherProbe = Gen::CallFunc( 'seraph_accel\\OnAsyncTasksPushByRe' );
			if( $sPhpLauncherProbe && is_string( $sPhpLauncherProbe ) )
	...
	static function _GetPhpLauncherEx( $sExtensionDir, $sPhpOs )
	{
		...
		if( 0 ) {}
		else if( preg_match( '@/php/?\\d+/(lib/php/(?:ext|mod))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
