...
Contributors: senols
Tags: chatgpt, gpt, openai, ai, chatbot, speech, whisper, image generator, gpt4
...
PDF Chat:
- The JavaScript file for the PDF Chat feature is now loaded only when the chat bot is actively in use. This optimizes performance by reducing unnecessary loading of resources.

WooCommerce:
- New feature added for custom prompts: You can now include `[current_short_description]` and `[current_full_description]` shortcodes in your custom prompts. These shortcodes will dynamically pull the current short and full descriptions of your product, respectively, allowing for more personalized and detailed content generation. Read documentation [here](https://docs.aipower.org/docs/woocommerce#customizing-prompts).
...
- **Batch Number Display**: AutoGPT - Queue tab now displays the total number of batches, including the number of pending and completed batches. This feature provides a quick overview of the batch processing status.
- **Queue Filtering**: Added the ability to filter the queue by status. You can now easily sort and view batches based on their completion status under AutoGPT - Queue.
- **Batch Name Truncation**: For improved readability, batch names are now truncated under AutoGPT - Queue.
- **Batch ID**: Added a Batch ID for each batch in the Queue tab. This makes it easier to identify and reference specific batches.

ChatGPT

- **Hide/Display Advanced Parameters**: Added option to toggle visibility of advanced AI parameters for a cleaner user interface.
- **Sync Button**: Added Sync button for custom models in ChatGPT shortcode.

Translation

- **POT file update**: Update POT file for Settings - Welcome tab and added translator comments.

Bug Fixes
- Fixed a bug in voice pitch validation.

= 1.7.40 =

# Improvements

- In the Content Writer - Custom Mode, you can now create content not just for posts and pages, but also for your custom post types. Your custom post types will automatically appear in the post type dropdown list.
- Replaced "stripslashed" for "wp_unslash" and "addslashes" for "wp_slash"
...
- We have a new icon for the chat widget.
- The conversation memory for the chat bot has been increased from 20 to 50, courtesy of the gpt-3.5-turbo-16k context window.
- Additional menu items have been added to the AI Assistant Wizard.
...
- Fixed an issue with the chat widget.
- Minified some JS files.
- Renamed the downloaded chat history file to chat.txt.

= 1.7.34 =

- New users are now being redirected to the Setup Wizard.

= 1.7.33 =

- [NEW]: GPT-3.5-Turbo-16K is now available.
- [FIX]: Better error handling for Playground.

= 1.7.32 =

- Better error handling for Playground, AI Forms and PromptBase.

= 1.7.31 =

- We now have a Quick Setup Wizard for onboarding new users. You can find it in the Help section of the plugin menu.

= 1.7.30 =

- Fixed an issue in PromptBase.
