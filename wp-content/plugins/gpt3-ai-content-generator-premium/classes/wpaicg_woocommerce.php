            ...
            if(
                ...
                if($step === 'description'){
                    if (!empty($short_description) && !empty($full_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_description;
                    } else if (!empty($short_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_description_no_full_description;
                    } else if (!empty($full_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_description_no_short_description;
                    } else {
                        $prompt = $wpaicg_woo_custom_prompt_description_no_descriptions;
                    }
                    $prompt = str_replace('[current_short_description]', $short_description, $prompt);
                    $prompt = str_replace('[current_full_description]', $full_description, $prompt);
                    $prompt = sprintf($prompt, $title);
                }
                if($step === 'short'){
                    if (!empty($short_description) && !empty($full_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_short;
                    } else if (!empty($short_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_short_no_full_description;
                    } else if (!empty($full_description)) {
                        $prompt = $wpaicg_woo_custom_prompt_short_no_short_description;
                    } else {
                        $prompt = $wpaicg_woo_custom_prompt_short_no_descriptions;
                    }
                    $prompt = str_replace('[current_short_description]', $short_description, $prompt);
                    $prompt = str_replace('[current_full_description]', $full_description, $prompt);
                    $prompt = sprintf($prompt, $title);
