...
class Sequence extends Model
{
    ...
    public function guessScheduledTime($sequenceEmail, $offsetSeconds = 0)
    {
        ...
        if (!is_array($sendingTimes)) {
            ...
            if (!$sendingTimes || count($sendingTimes) != 2) {
                ...
                $sendingDate = $this->maybeAddOffsetSeconds($sendingDate, $offsetAdded, $offsetSeconds);
    ...
    private function maybeAddOffsetSeconds($date, $offsetAdded, $offsetSeconds)
    {
        if (!$offsetAdded && $offsetSeconds) {
            $date = date('Y-m-d', strtotime($date) + $offsetSeconds);
        }
