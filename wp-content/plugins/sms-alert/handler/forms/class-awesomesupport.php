...
class SaAwesomeSupport extends FormInterface
{
    ...
    private $form_session_var2 = FormSessionVars::AWESOME_SUPPORT_DEFAULT_REG;
    ...
    public function siteRegistrationErrors($data, $update, $user_id, $userdata)
    {
        ...
        if (!$update ) {
            ...
            if (isset($_REQUEST['option']) && 'smsalert_wpas_form_otp' === sanitize_text_field(wp_unslash($_REQUEST['option']))) {
                SmsAlertUtility::initialize_transaction($this->form_session_var);
            } else if (! empty($_POST['wpas_billling_phone']) ) {                
    ...
    public function handle_failed_verification($userLogin, $userEmail, $phoneNumber)
    {
        ...
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            smsalert_site_otp_validation_form($userLogin, $userEmail, $phoneNumber, SmsAlertUtility::_get_invalid_otp_method(), 'phone', false);
        }
