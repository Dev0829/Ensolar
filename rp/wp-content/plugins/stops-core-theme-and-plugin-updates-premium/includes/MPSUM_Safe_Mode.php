...
class MPSUM_Safe_Mode {
	...
	/**
	 * Already logged plugins to avoid duplicate logging
	 *
	 * @var array
	 */
	private $already_logged_plugins = array();
	
	/**
	 * Already logged themes to avoid duplicate logging
	 *
	 * @var array
	 */
	private $already_logged_themes = array();
	...
	public function perform_api_check($plugin_file) {
		...
		if (false !== $plugin_safe_mode_transient && is_object($plugin_safe_mode_transient)) {
			...
			if (!is_wp_error($response)) {
				$plugin_item = maybe_unserialize(wp_remote_retrieve_body($response));
				if (is_null($plugin_item)) {
	...
	public function maybe_automatic_updates_plugins($update, $item) {
		...
		foreach ($this->already_logged_plugins as $plugin) {
			if (isset($plugin->id)) {
		...
		if (!$this->is_compatible_wp_version($item) || (defined('EUM_SAFE_MODE_DEBUG') && true === EUM_SAFE_MODE_DEBUG)) {
			...
			$this->already_logged_plugins[] = $item;
			return false;
		}
	...
	public function maybe_automatic_updates_themes($update, $item) {
		...
		// Prevent logging twice
		foreach ($this->already_logged_themes as $theme) {
			if (isset($theme->theme)) {
				if ($theme->theme === $item->theme) {
					return false;
				}
			}
		}
		...
		if (!$this->is_compatible_php_version($item) || (defined('EUM_SAFE_MODE_DEBUG') && true === EUM_SAFE_MODE_DEBUG)) {
			...
			$this->already_logged_themes[] = $item;
			return false;
		}
