	...
	public function perform_api_check($plugin_file, $force = false) {
		...
		if (false !== $plugin_unmaintained_transient && is_object($plugin_unmaintained_transient) && ! $force) {
			...
			if (! is_wp_error($plugin_item)) {
				...
				// Set transient for potential third-party plugin that isn't recognized by the API.
				set_site_transient('eum_plugin_unmaintained_' . $plugin_file, new stdClass(), 6 * 60 * 60);
				return new stdClass();
			}
		}
		return new stdClass();
