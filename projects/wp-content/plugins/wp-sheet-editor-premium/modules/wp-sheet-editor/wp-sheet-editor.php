...
if ( ! class_exists( 'WP_Sheet_Editor' ) ) {
	...
	class WP_Sheet_Editor {
		...
		function get_option( $key, $value, $default = null ) {
			$out = null;
		...
		function register_menu() {
			if ( apply_filters( 'vg_sheet_editor/register_admin_pages', true ) ) {
				add_menu_page( __( 'WP Sheet Editor', 'vg_sheet_editor' ), __( 'WP Sheet Editor', 'vg_sheet_editor' ), 'manage_options', 'vg_sheet_editor_setup', array( $this, 'render_quick_setup_page' ), VGSE()->plugin_url . 'assets/imgs/icon-20x20.png' );
