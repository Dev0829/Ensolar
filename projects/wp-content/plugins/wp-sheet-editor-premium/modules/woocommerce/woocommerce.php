...
//				'post_status' => 'published',
		...
		function include_sku_in_search_by_keyword_check($check, $single_keyword, $clauses, $raw_keywords, $operator, $internal_join) {
			...
			if ($post_type === $this->post_type) {
				$check .= " $internal_join lookup.sku $operator '%" . esc_sql($single_keyword) . "%' ";
			}
		...
		function get_product_type($product_id) {
			$types = wp_get_object_terms($product_id, 'product_type', array(
		...
		function register_columns($editor) {
			...
			$editor->args['columns']->register_item('post_author', $post_type, array(
				'data_type' => 'post_data',
				'column_width' => 120,
			...
			$editor->args['columns']->register_item($product_type_tax, $post_type, array(
				...
				'supports_formulas' => true,
				'formatted' => array('data' => $product_type_tax, 'editor' => 'select', 'selectOptions' => VGSE()->data_helpers->get_taxonomy_terms($product_type_tax))
			));
			...
			$editor->args['columns']->register_item('_stock_status', $post_type, array(
				...
				'supports_formulas' => true,
				'formatted' => array(
					'editor' => 'select', 'selectOptions' => $stock_statuses
				),
				// We must use the slow execution method to sync with the lookup table				
				'supports_sql_formulas' => false,
			));
